...
void main()
{
    ...
    if (patchSize.x <= 0.5)
    {
        patchCoords.x = (i_texCoords.x*patchSize.x);
        if (i_texCoords.x >= 0.5) { patchCoords.x += (1.0-patchSize.x); }
    }
    if (patchSize.y <= 0.5)
    {
        patchCoords.y = (i_texCoords.y*patchSize.y);
        if (i_texCoords.y >= 0.5) { patchCoords.y += (1.0-patchSize.y); }
    }
